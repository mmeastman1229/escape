...
def pick_up_object():
    ...
    if item_player_is_on in items_player_may_carry:
        ...
        show_text(
            f'>> PICKUP: "Now carrying {objects[item_player_is_on][3]}"', 0
        )
        sounds.pickup.play()
        time.sleep(0.5)
    else:
...
def drop_object(old_y, old_x):
    ...
    if room_map[old_y][old_x] in [0, 2, 39]:  # places you can drop things
        ...
        show_text(
            f'>> DROP: "You have dropped {objects[item_carrying][3]}" ', 0)
        sounds.drop.play()
...
def remove_object(item):  # Take item out of inventory
    ...
    for prop_number, details in props.items():
        ...
        if details[0] == current_room:  # if prop is in the room
            ...
            if (
                ...
                description = f'>> EXAMINE: "You found ' \
...
def use_object():
    ...
    standard_responses = {
        4: f'>> MESSAGE: "Air is running out!'
    ...
    if item_carrying == 70 or item_player_is_on == 70:
        ...
        if not suit_stitched:
            use_message += f'>> SYSTEM INFO: "*ALERT* SUIT FABRIC TORN/"'
        if not air_fixed:
            use_message += f'>> SYSTEM INFO: "*ALERT* SUIT AIR BOTTLE MISSING"'
        if suit_stitched and air_fixed:
            use_message += f'>> SYSTEM INFO: "SUIT OK"'
        ...
        use_message = f'>> MESSAGE: "You use {objects[56][3]} ' \
                f'to repair the suit fabric" '
        suit_stitched = True
        remove_object(58)
        ...
        show_text(f'Mission success! You have made it to safety"', 1)
        game_over = True
        ...
        show_text('>> SYSTEM ALERT: "Door to engineering bay is open for'
                  '60 seconds', 1)
    ...
    if suit_stitched and air_fixed:  # open airlock access
        if current_room == 31 and props[20][0] == 31:
            ...
            show_text(
                '>> MESSAGE: "The computer tells you the airlock'
                'is now open." ', 1)
...
def game_completion_sequence():
    ...
    if launch_frame < 9:
        ...
        screen.surface.set_clip(None)
        screen.draw.text("MISSION", (200, 380), color=WHITE_SMOKE,
                         fontsize=128, shadow=(1, 1), scolor=BLACK)
        screen.draw.text("COMPLETE", (145, 480), color=WHITE_SMOKE,
                         fontsize=128, shadow=(1, 1), scolor=BLACK)
...
clock.schedule_interval(display_inventory, 0.05)
