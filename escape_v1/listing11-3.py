...
def pick_up_object():
    ...
    if item_player_is_on in items_player_may_carry:
        ...
        show_text(f'>> PICKUP: "Now carrying {objects[item_player_is_on][3]}"', 1)
        sounds.pickup.play()
        time.sleep(2)
    else:
    ...
    for prop_number, details in props.items():
        ...
        if details[0] == current_room:  # if prop is in the room
            ...
            if (
                ...
                description = f">> EXAMINE: You found {objects[prop_number][3]}"
                sounds.combine.play()
...
def use_object():
    ...
    if item_carrying == 70 or item_player_is_on == 70:
        ...
        show_text(">> SYSTEM ALERT: Door to engineering bay is open for 60 seconds...", 1)
    ...
    if suit_stitched and air_fixed:  # open airlock access
        if current_room == 31 and props[20][0] == 31:
            ...
            sounds.say_airlock_open.play()
            show_text(">> MESSAGE: The computer tells you the airlock is now "
                      "open.", 1)
...
def testing_info():
    global room_map, room_width, room_height, room_name, player_x, player_y
    global objects, item_carrying, current_room
    global door_frames, door_shadow_frames
    global door_frame_number, door_object_number
    print(
        f"""
        ...
        Door Ojbect number: {door_object_number}
    """
    )
