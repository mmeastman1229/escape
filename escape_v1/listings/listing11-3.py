...
Its black box has a radio sealed inside.",
    ...
    72: [
        images.radio,
        None,
        "A radio communications system, from the \
Poodle",
...
Needs a GPS module.",
    ...
    76: [
        images.scissors,
        None,
        "Scissors. They're too blunt to cut \
anything. Can you sharpen them?",
        "blunt scissors",
    ],
...
def start_room():
    show_text("You are here: " + room_name, 1)
...
def pick_up_object():
    ...
    if item_player_is_on in items_player_may_carry:
        ...
        show_text("Now carrying " + objects[item_player_is_on][3], 1)
        sounds.pickup.play()
        time.sleep(0.5)
    else:
...
def drop_object(old_y, old_x):
    ...
    if room_map[old_y][old_x] in [0, 2, 39]:  # places you can drop things
        ...
        show_text("You have dropped " + objects[item_carrying][3], 1)
        sounds.drop.play()
        remove_object(item_carrying)
        time.sleep(0.5)
    else:  # This only happens if there is already a prop here
        show_text("You can't drop that there.", 1)
        time.sleep(0.5)
...
Rendezvous Sector 13, outside."
        ...
        show_text("Congratulations, " + PLAYER_NAME + "!", 1)
        show_text("Mission success! You have made it to safety.", 2)
        game_over = True
        ...
        show_text("Door to engineering bay is open for 60 seconds", 2)
    ...
    if suit_stitched and air_fixed:  # open airlock access
        if current_room == 31 and props[20][0] == 31:
            ...
            show_text("The computer tells you the airlock is now open.", 2)
        elif props[20][0] == 31:
    ...
    for recipe in RECIPES:
        ...
        if (item_carrying == ingredient1 and item_player_is_on == ingredient2) or (
            ...
            use_message = (
                "You combine "
                + objects[ingredient1][3]
                + " and "
                + objects[ingredient2][3]
                + " to make "
...
def do_door_animation():
    ...
    door_frame_number += 1
    assert door_frame_number < 0, "Assert Test"
