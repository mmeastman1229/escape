...
def pick_up_object():
    ...
    if item_player_is_on in items_player_may_carry:
        ...
        show_text(f'>> PICKUP: "Now carrying {objects[item_player_is_on][3]}"', 0)
        sounds.pickup.play()
        time.sleep(0.5)
    else:
...
def display_inventory():
    ...
    screen.draw.filled_rect(box, DARK_GREY)
    ...
    for prop_number, details in props.items():
        ...
        if details[0] == current_room:  # if prop is in the room
            ...
            if (
                ...
                description = f'>> EXAMINE: "You found ' f'{objects[prop_number][3]}"'
                sounds.combine.play()
...
def use_object():
    ...
    if item_carrying == 70 or item_player_is_on == 70:
        ...
        use_message = (
            f'>> MESSAGE: "You use {objects[56][3]} ' f'to repair the suit fabric" '
        )
        suit_stitched = True
        remove_object(58)
        ...
        show_text(f'>> MESSAGE: "Congratulations {PLAYER_NAME}', 1)
        show_text(f'Mission success! You have made it to safety"', 2)
        game_over = True
    ...
    if suit_stitched and air_fixed:  # open airlock access
        if current_room == 31 and props[20][0] == 31:
            ...
            show_text(
                '>> MESSAGE: "The computer tells you the airlock' 'is now open." ', 2)
...
clock.schedule_interval(display_inventory, 0.25)
