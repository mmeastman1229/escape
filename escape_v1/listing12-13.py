...
# Main gameplay variables used in game
...
# Functions and variables used to create map
...
# All objects that exist within the game
...
objects = {
    ...
    22: [
        images.door,
        images.door_shadow,
        f"A locked door. It needs {PLAYER_NAME}'s access card",
    ],
    23: [
        images.door,
        images.door_shadow,
        f"A locked door. It needs {FRIEND1_NAME}'s access card",
    ],
    24: [
        images.door,
        images.door_shadow,
        f"A locked door. It needs {FRIEND2_NAME}'s access card",
    ],
    25: [
        images.door,
        images.door_shadow,
    ...
    64: [
        images.rags,
        None,
        "An oily rag. Pick it up by one corner if you must!",
        "an oily rag",
    ],
    65: [
        images.hammer,
...
assert checksum == 208011, "Error in scenery data"
...
def start_room():
    ...
    show_text(f">> LOCATION: {room_name}", 0)
...
# Main draw functions for pygame
...
def draw_player():
    ...
    draw_shadow(
        player_image_shadow, player_y + player_offset_y, player_x + player_offset_x
    )
...
def show_text(text_to_show, line_number):
    ...
    text_lines = [15, 50]
    box = Rect(
        (0, text_lines[line_number]),
        (800, 35)
    )
    screen.draw.filled_rect(box, BLACK)
    screen.draw.text(text_to_show, (20, text_lines[line_number]), color=YELLOW)
...
# Props are objects that move between rooms, appear or disappear.
...
print(f"Prop checksum {checksum}")
...
print(f"Recipe checksum: {checksum}")

###############################################################################

##   PROP INTERACTIONS   ##

# Functions used for interacting with props within the game (movable items)
...
def pick_up_object():
    ...
    if item_player_is_on in items_player_may_carry:
        ...
        show_text(">> PICKUP: Now carrying " f"{objects[item_player_is_on][3]}", 1)
        sounds.pickup.play()
        time.sleep(2)
    else:
        show_text(">> SYSTEM ERROR: You can't carry that!", 0)
...
def display_inventory():
    ...
    screen.draw.text(description, (20, 130), color=YELLOW)
...
def drop_object(old_y, old_x):
    ...
    if room_map[old_y][old_x] in [0, 2, 39]:  # places you can drop things
        ...
        show_text(f">> DROP: You have dropped {objects[item_carrying][3]}", 0)
        sounds.drop.play()
        remove_object(item_carrying)
        time.sleep(0.5)
    else:
        show_text(">> SYSTEM ERROR: You can't drop that there", 0)
        time.sleep(0.5)


def remove_object(item):  # Take item out of inveinntory
    ...
    else:  # Otherwis set it to the new selected item
        item_carrying = in_my_pockets[selected_item]
    display_inventory()
...
def examine_object():
    ...
    if item_player_is_on in [0, 2]:  # Don't describe the floor
        return
    description = f">> EXAMINE: {objects[item_player_is_on][2]}"
    for prop_number, details in props.items():
        ...
        if details[0] == current_room:  # if prop is in the room
            # If prop is hidden (= at players location but not on map)
            if (
                details[1] == player_y
                ...
                description = ">> EXAMINE: You found " f"{objects[prop_number][3]}"
                sounds.combine.play()
...
# Functions used for using objects within the game environment
...
def use_object():
    ...
    standard_responses = {
        4: ">> ALERT: Air is running out! You can't take this lying down!'",
        6: ">> MESSAGE: This is no time to sit around!",
        7: ">> MESSAGE: This is no time to sit around!",
        32: ">> MESSAGE: It shakes and rumbles, but nothing else happens",
        34: ">> MESSAGE: Ah! That's better.Now wash your hands",
        35: ">> MESSAGE: You wash your hands and shake the water off",
        37: ">> MESSAGE: The test tubes smoke slightly as you shake them",
        54: ">> MESSAGE: You chew the gum. It's sticky like glue",
        55: ">> MESSAGE: The yoyo bounces up and down, "
        "slightly slower than on earth",
        56: ">> MESSAGE: It's a bit too fiddly. " "Can you thread it on something",
        59: ">> MESSAGE: You need to fix the leak" "before you can use the canister",
        61: ">> MESSAGE: You try signalling with the mirror," "but nobody can see you",
        62: ">> MESSAGE: Don't throw resources away." "Things may come in handy...",
        67: ">> MESSAGE: To enjoy yummy space food, just add water!",
        75: f">> MESSAGE: You are in Sector: {str(current_room)} "
        f" //X: {str(player_x)}  //Y: {str(player_y)}",
    }
    ...
    if item_carrying == 70 or item_player_is_on == 70:
        use_message = '>> MESSAGE: "Banging tunes!"'
        sounds.steelmusic.play(2)

    elif item_player_is_on == 11:
        use_message = f">> SYSTEM INFO: AIR: {air}% / ENERGY {energy}% /"

        if not suit_stitched:
            use_message += f" *ALERT* SUIT FABRIC TORN / "
        if not air_fixed:
        ...
        use_message = f">> MESSAGE: You fix {objects[60][3]} to the suit"
        air_fixed = True
        air = 90
        air_countdown()
        remove_object(60)
        ...
        use_message = (
            f">> MESSAGE: You use {objects[56][3]} to repair" " the suit fabric"
        )
        suit_stitched = True
        remove_object(58)
        ...
        use_message = (
            ">> MESSAGE: You radio for help. "
            "A rescue ship is coming. Rendezvous Sector 13, outside"
        )
        props[40][0] = 13

    elif (
        item_carrying == 66 or item_player_is_on == 66
    ) and current_room in outdoor_rooms:
        use_message = ">> MESSAGE: You dig..."
        if (
            current_room == LANDER_SECTOR
            and player_x == LANDER_X
            and player_y == LANDER_Y
        ):
            add_object(71)
            use_message = ">> MESSAGE: You found the Poodle Lander!"

    elif item_player_is_on == 40:
        clock.unschedule(air_countdown)
        show_text(f">> MESSAGE: Congratulations {PLAYER_NAME}!", 0)
        ...
        if energy > 100:
            energy = 100
        use_message = (
            ">> MESSAGE: You munch the lettuce " "and get a little energy back"
        )
        draw_energy_air()
        ...
        if current_room == 27:
            open_door(26)
        props[25][0] = 0  # Door from RM 32 to engineering bay
        props[26][0] = 0  # Door inside engineering bay
        clock.schedule_unique(shut_engineering_door, 60)
        use_message = ">> MESSAGE: You press the button"
        show_text(
            ">> SYSTEM ALERT: Door to engineering bay " "is open for 60 seconds...", 1
        )
        ...
        energy = 100
        use_message = f">> MESSAGE: You use the food to restore your energy"
        remove_object(68)
        draw_energy_air()
    ...
    if suit_stitched and air_fixed:  # open airlock access
        if current_room == 31 and props[20][0] == 31:
            ...
            show_text(
                ">> MESSAGE: The computer tells you the airlock is " "now open.", 1
            )
        elif props[20][0] == 31:
            props[20][20] == 0  # remove door from map
            sounds.say_airlock_open.play()
            show_text(
                ">> MESSAGE: The computer tells you " "the airlock is now open.", 1
            )
    ...
    for recipe in RECIPES:
        ...
        if (item_carrying == ingredient1 and item_player_is_on == ingredient2) or (
            item_carrying == ingredient2 and item_player_is_on == ingredient1
        ):
            use_message = (
                ">> MESSAGE: "
                f"You combine {objects[ingredient1][3]} "
                f"and {objects[ingredient2][3]} "
                f"to make {objects[combination][3]}"
            )
    ...
    if item_carrying in ACCESS_DICTIONARY:
        ...
        if props[door_number][0] == current_room:
            use_message = ">> MESSAGE: You unlock the door"
            sounds.say_doors_open.play()
            sounds.doors.play()
...
def game_completion_sequence():
    global launch_frame  # initial value is 0, set up in the VARIABLES section
    box = Rect((0, 150), (1000, 600))
    screen.draw.filled_rect(box, (128, 0, 0))
    box = Rect((0, top_left_y - TILE_SIZE), (800, 390))
    screen.surface.set_clip(box)
...
# Special functions for working with doors
...
def close_door(closing_door_number):
    ...
    if player_y == props[door_object_number][1]:
        if player_y == 0:  # if in top of doorway
...
def shut_engineering_door():
    ...
    if current_room == 32:
        close_door(25)
    show_text(">> SYSTEM INFO: The computer tells " "you the doors are closed.", 1)
...
def door_in_room_26():
    ...
    if ((player_y == 8 and player_x == 2) or props[63] == [26, 8, 2]) and props[21][
        0
    ] == 26:
    ...
    if (
        (player_y != 8 or player_x != 2) and props[63] != [26, 8, 2]
...
# These are all the functions that have to do with air and energy
...
def air_countdown():
    ...
    draw_energy_air()
    if air < 1:
        end_the_game(">> DEATH: You're out of air!")


def alarm():
    show_text(
        f">> ALERT: Air is running out {PLAYER_NAME}! "
...
# All the items in the game including functions that deplete energy
...
def deplete_energy(penalty):
    ...
    draw_energy_air()
    if energy < 1:
        end_the_game(">> DEATH: You're out of energy!")
...
def hazard_move():
    ...
    for hazard in current_room_hazards_list:
        ...
        if (hazard_y == player_y and hazard_x == player_x) or (
            hazard_y == from_player_y and hazard_x == from_player_x and
                player_frame > 0
                ):
            sounds.ouch.play()
            deplete_energy(10)
        ...
        # Stop when hazard hits sceenary or another hazard.
        if (
            room_map[hazard_y][hazard_x] not in items_player_may_stand_on
...
# functions used to start the game when run
...
# A higher number below givesw a longer time limit
