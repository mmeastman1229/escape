...
def show_text(text_to_show, line_number):
    ...
    screen.draw.text(text_to_show, 
                     (20, text_lines[line_number]), color=YELLOW)
...
def pick_up_object():
    ...
    if item_player_is_on in items_player_may_carry:
        ...
        show_text(">> PICKUP: Now carrying "
                  f"{objects[item_player_is_on][3]}", 1)
        sounds.pickup.play()
        time.sleep(2)
    else:
    ...
    for prop_number, details in props.items():
        ...
        if details[0] == current_room:  # if prop is in the room
            ...
            if (
                ...
                description = ">> EXAMINE: You found " \
                    f"{objects[prop_number][3]}"
                sounds.combine.play()
...
def use_object():
    ...
    use_message = ">> MESSAGE: You fiddle around with it " \
    ...
    standard_responses = {
        ...
        61: ">> MESSAGE: You try signalling with the mirror,"
            "but nobody can see you",
        62: ">> MESSAGE: Don't throw resources away."
    ...
    if item_carrying == 70 or item_player_is_on == 70:
    ...
    elif (item_carrying == 58
            or item_player_is_on) == 58 and not suit_stitched:
        use_message = f">> MESSAGE: You use {objects[56][3]} to repair" \
            " the suit fabric"
        suit_stitched = True
        remove_object(58)

    elif item_carrying == 72 or item_player_is_on == 72:
        use_message = ">> MESSAGE: You radio for help. " \
        ...
        if (current_room == LANDER_SECTOR
                and player_x == LANDER_X and player_y == LANDER_Y):
            add_object(71)
        ...
        if energy > 100:
            energy = 100
        use_message = ">> MESSAGE: You munch the lettuce " \
        ...
        show_text(">> SYSTEM ALERT: Door to engineering bay "
                  "is open for 60 seconds...", 1)
    ...
    if suit_stitched and air_fixed:  # open airlock access
        if current_room == 31 and props[20][0] == 31:
            ...
            show_text(">> MESSAGE: The computer tells you the airlock is "
                      "now open.", 1)
        elif props[20][0] == 31:
            props[20][20] == 0  # remove door from map
            sounds.say_airlock_open.play()
            show_text(">> MESSAGE: The computer tells you "
                      "the airlock is now open.", 1)
    ...
    for recipe in RECIPES:
        ...
        if ((
            item_carrying == ingredient1
            and item_player_is_on == ingredient2
            )
            or (
                item_carrying == ingredient2
...
def shut_engineering_door():
    ...
    if current_room == 32:
        close_door(25)
    show_text(">> SYSTEM INFO: The computer tells "
              "you the doors are closed.", 1)
...
def door_in_room_26():
    ...
    shadow_frames = [
        images.door_shadow,
        images.door_shadow_1,
        images.door_shadow_2,
        images.door_shadow_3,
        images.door_shadow_4,
        None
    ]
